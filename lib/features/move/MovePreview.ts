...
export default class MovePreview{
  ...
  function getAllDraggedElements(shapes:any) {
    ...
    let  allConnections = map(allShapes, function(shape:any) {
      return (shape.incoming || []).concat(shape.outgoing || []);
    });

   let boolAr:boolean[]= [true];
   let mixAr:any[]=allShapes.concat(allConnections);
   return mixAr.concat(boolAr);
  }
  ...
  function setMarker(element:any, marker:any) {
    ...
    [ MARKER_ATTACH, MARKER_OK, MARKER_NOT_OK, MARKER_NEW_PARENT ].forEach(function(m:any) {
  ...
  eventBus.on('shape.move.start', LOW_PRIORITY, function(event:any) {
    ...
    let  context :any = event.context,
        dragShapes:any = context.shapes,
        allDraggedElements :any = context.allDraggedElements;

    let  visuallyDraggedShapes = getVisualDragShapes(dragShapes);

    if (!context.dragGroup) {
      let  dragGroup = svgCreate('g');
      svgAttr(dragGroup, styles.cls('djs-drag-group', [ 'no-events' ]));

      let  defaultLayer :any= canvas.getDefaultLayer();
    ...
    if (!allDraggedElements) {
      allDraggedElements = getAllDraggedElements(dragShapes);
       ...
       allDraggedElements = allDraggedElements.concat( getAllDraggedElements(dragShapes));
    }
...
function removeEdges(elements:any) {
  ...
  let  filteredElements = filter(elements, function(element:any):any {
