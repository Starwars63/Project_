...
export default class MoveEvents{
  start:any;
  constructor(eventBus:any, dragging:any, modeling:any,
  ...
  eventBus.on('shape.move.start', MEDIUM_PRIORITY, function(event:any) {
    ...
    let  context:any = event.context,
        validatedShapes :any= context.validatedShapes,
        canExecute:any;

    canExecute = context.canExecute = canMove(validatedShapes,null,null,null);
  ...
  eventBus.on('shape.move.end', function(event:any) {
  ...
  eventBus.on('element.mousedown', function(event:any) {
    ...
    let  originalEvent:any = getOriginalEvent(event);

    if (!originalEvent) {
      throw new Error('must supply DOM mousedown event');
    }

    return start(originalEvent, event.element,null);
  });


  function start(event:any, element:any, activate:any) {
    ...
    let  referencePoint:any = mid(element);
